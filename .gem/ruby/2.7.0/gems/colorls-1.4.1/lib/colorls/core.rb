...
module ColorLS
  class Core
    ...
    def format_mode_bold(rwx, special, char)
      m_r = (rwx & 4).zero? ? '-' : 'r'
      m_w = (rwx & 2).zero? ? '-' : 'w'
      m_x = if special
              (rwx & 1).zero? ? char.upcase : char
      ...
      @modes[m_r].bold() + @modes[m_w].bold() + @modes[m_x].bold()
    end

    def mode_info(stat)
      m = stat.mode

      format_mode_bold(m >> 6, stat.setuid?, 's') +
        format_mode(m >> 3, stat.setgid?, 's') +
        format_mode(m, stat.sticky?, 't')
    end

    def user_info(content)
      content.owner.ljust(@userlength, ' ').colorize(@colors[:user]).bold()
    end
    ...
    def size_info(filesize)
      ...
      ud = size[1][0,1].downcase
      size = "\e[38;5;244;1m#{size[0][0..-4].rjust(3,' ')}\e[0m\e[38;5;244m#{ud.ljust(1,' ')}"

      size
    end

    def mtime_info(file_mtime)
      mtime = file_mtime.strftime("%d, %b  [%H:%M] ")
      now = Time.now
    ...
    def long_info(content)
      ...
      [mode_info(content.stats), size_info(content.size),
       user_info(content), mtime_info(content.mtime)].join('  ')
    end
